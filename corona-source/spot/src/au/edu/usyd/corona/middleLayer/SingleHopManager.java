 ...
 * If a route is invalidated 3 times, we throw a noRouteException, meaning a
 * node is probably not actually 1 hop away.
 * 
 * Since A RouteInfo object is never freed we must create one the first time it
 * is requested. We then store that reference in a Hashtable. The next time it
 * is requested, we returned the caches RouteInfo object. The intention is that
 * this will minimize garbage collection, as well as serve as a sample for
 * writing simple routing managers.
 * 
 * This code is modified to cause the LowPan to throw an exception after a
 * single failure in sending a packet, as we do our own checks for this.
 * 
...
public class SingleHopManager implements IRoutingManager {
	...
	private final Hashtable invalidRoutes;
	
	private static final int MAX_FAILURES = 1;
	
	/**
	 * Creates a new instance of SingleHopManager The SingleHopManager can be
	 * used by the LowPan layer to limit communications to nodes within a single
	 * radio hop away.
	 */
	...
	/**
	 * lookup a route to this address. The answer will always be that the
	 * destination address is exactly 1 hop away
	 * 
	...
	/**
	 * registers a listener for routing messages. Single hop has no messages so
	 * this is a NO OP.
	 * 
	...
	/**
	 * deregisters a listener for routing messages. Single hop has no messages so
	 * this is a NO OP.
	 * 
	...
	/**
	 * retrieve routing information for a destination address. Answer is always
	 * that the address is one hop away.
	 * 
	 * @param address destination address of route
	 * @return returns a route info object where nexthop is the destination and
	 * is 1 hop away
	 */
